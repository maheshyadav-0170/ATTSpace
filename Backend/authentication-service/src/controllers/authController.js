...
async function authMiddleware(req, res, next) {
  ...
  try {
    ...
    if (blacklisted) {
      logger.info("authMiddleware: Token is blacklisted");
      return res
        .status(401)
        .json({ message: "Unauthorized: Token is no longer valid" });
    }
...
async function checkAttuid(req, res) {
  ...
  if (!attuid || typeof attuid !== "string") {
    ...
    return res
      ...
      .json({
        message:
          "The provided attuid is invalid. Please enter a valid attuid and try again.",
      });
  }
  ...
  try {
    ...
    if (cachedEmployee) {
      ...
      logger.info(
        `check-attuid: Employee found in Redis cache for attuid: ${attuid}`
      );
    } else {
      ...
      if (employee) {
        logger.info(
          `check-attuid: Employee fetched from MongoDB for attuid: ${attuid}`
        );
    ...
    if (!employee) {
      ...
      return res
        ...
        .json({
          message:
            "No employee record was found for the provided attuid. Please verify and try again.",
        });
    }
    ...
    if (authData) {
      ...
      return res.json({
        next: "password",
        message:
          "A verified user account exists. Please provide your password to continue.",
      });
    }

    // Step 3: No authuser — send OTP
    ...
    logger.info(
      `check-attuid: OTP generated and sent to email for attuid: ${attuid}`
    );
    return res.json({
      next: "otp",
      message:
        "A one-time password (OTP) has been sent to your registered company email address. Please check your inbox.",
    });
  } catch (err) {
    logger.error(
      `checkAttuid: Error processing attuid ${attuid}: ${err.toString()}`
    );
    return res
      .status(500)
      .json({
...
async function verifyOtp(req, res) {
  ...
  if (!attuid || !otp) {
    ...
    return res
      ...
      .json({
        message:
          "Both attuid and OTP are required to proceed with verification.",
      });
  }
  ...
  try {
    ...
    if (!stored) {
      ...
      return res
        ...
        .json({
          message:
            "The OTP has either expired or was not found. Please request a new OTP.",
        });
    }
    if (stored !== String(otp)) {
      ...
      return res
        ...
        .json({
          message:
            "The provided OTP is invalid. Please enter the correct OTP and try again.",
        });
    }
    ...
    if (!employee) {
      ...
      return res
        .status(400)
        .json({
          message: "No employee record was found for the provided attuid.",
        });
    }
    ...
    await AuthUser.findOneAndUpdate({ attuid }, update, {
      upsert: true,
      new: true,
    ...
    logger.info(
      `verify-otp: Account verified and created for attuid: ${attuid}`
    );
    return res.json({
      next: "set-password",
      message:
        "OTP verification successful. Please set a new password for your account.",
    });
  } catch (err) {
    logger.error(
      `verify-otp: Error verifying OTP for attuid ${attuid}: ${err.toString()}`
    );
    return res
      .status(500)
      .json({
        message:
          "An unexpected server error occurred during OTP verification. Please try again later.",
      });
  }
}
...
async function setPassword(req, res) {
  ...
  if (!attuid || !password) {
    ...
    return res
      ...
      .json({
        message:
          "Both attuid and a valid password are required to set a new password.",
      });
  }
  ...
  try {
    ...
    if (!user) {
      ...
      return res
        .status(404)
        .json({
          message: "No user account was found for the provided attuid.",
        });
    }
    ...
    await user.save();
    logger.info(
      `set-password: Password successfully set for attuid: ${attuid}`
    );
    ...
    try {
      ...
      logger.warn(
        `set-password: Failed to cache user for attuid ${attuid}: ${cacheErr.toString()}`
      );
    }

    return res.json({
      message:
        "Your password has been successfully set. You can now log in with your credentials.",
    });
  } catch (err) {
    logger.error(
      `set-password: Error setting password for attuid ${attuid}: ${err.toString()}`
    );
    return res
      .status(500)
      .json({
        message:
          "An unexpected server error occurred while setting the password. Please try again later.",
      });
  }
}
...
async function login(req, res) {
  ...
  if (!attuid || !password) {
    ...
    return res
      .status(400)
      .json({ message: "Both attuid and password are required to log in." });
  }
  ...
  try {
    ...
    if (!user.password) {
      ...
      return res
        ...
        .json({
          message:
            "No password has been set for this account. Please set a password to log in.",
        });
    }
    ...
    logger.error(
      `login: Error during login for attuid ${attuid}: ${err.toString()}`
    );
    return res
      .status(500)
      .json({
...
async function logout(req, res) {
  ...
  if (!token) {
    ...
    return res
      .status(400)
      .json({ message: "A valid Bearer token is required to log out." });
  }
  ...
  try {
    ...
    if (!decoded || !decoded.exp) {
      ...
      return res
        .status(401)
        .json({
          message: "The provided token is either invalid or has expired.",
        });
    }
    ...
    if (alreadyBlacklisted) {
      logger.info(
        `logout: Token already blacklisted for attuid: ${decoded.attuid}`
      );
      return res
        .status(200)
    ...
    if (ttl > 0) {
      ...
      logger.info(
        `logout: Token blacklisted for ${ttl} seconds for attuid: ${decoded.attuid}`
      );
    }
    ...
    logger.error(
      `logout: Error during logout for token ${token}: ${err.toString()}`
    );
    return res
      .status(401)
      .json({
        message:
          "The provided token is invalid or has expired. Please try again.",
      });
  }
}
